  ...
  useEffect(() => {
    ...
    const offMenu = nativeApi.onMenuCommand?.(async (command: string) => {
      switch (command) {
        ...
        case "SAVE_DRAWING": {
          const payload = JSON.stringify({ version: "2.0.3", schemaVersion: 2, savedAt: new Date().toISOString(), currentFileName, activeLayoutId: activeLayout?.id || null, entities, layers, layouts, viewports, smartObjects, spreadsheetParameters, translationMemory, blockLibrary, dependencyEdges, modules, selectedWorkbench }, null, 2);
          ...
          <div className="w-full max-w-5xl rounded-2xl border border-neutral-700/80 bg-[#16181c] shadow-2xl overflow-hidden">
            <div className="px-8 py-7 border-b border-neutral-800 bg-gradient-to-r from-[#15181c] to-[#101820] flex items-center justify-between gap-6">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-bold text-white">HNL CAD AI <span className="text-cyan-400">v2.0.3</span></h1>
