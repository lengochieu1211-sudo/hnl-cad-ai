...
namespace HNL.VXT.Core.Preview
{
    ...
    internal static class VxtOrthogonalNotchRegionPlanner
    {
        ...
        private static bool TryReuseSharedGrid(
            RectRegion region,
            IReadOnlyList<double> sharedGrid,
            VxtSettings settings,
        ...
        {
            grid = null;
            if (sharedGrid == null || sharedGrid.Count == 0) return false;

            var projected = UniqueSort(sharedGrid.Where(y =>
                y >= region.Y1 - Tol && y <= region.Y2 + Tol));
            if (!GridValid(projected, region.Y1, region.Y2, settings)) return false;
